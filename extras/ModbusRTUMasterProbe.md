...
This example can be used to probe data on one or more Modbus RTU slave/server devices.

The exact details of how to wire up this example will depend on the device(s) you are connecting to.
An RS-232 or RS-485 transceiver may be needed. If you are using a half-duplex RS-485 transceiver, if possible, connect the RE and DE pins together.
...
Once the sketch is uploaded, open the serial monitor and set it to a baud rate of 9600. The prompt will guide you from there.

This example sketch is primarily intended to be a tool, not an example of how to use this library. For an example that is a little simpler, see [ModbusRTUMasterExample](https://github.com/CMB27/ModbusRTUMaster/blob/main/extras/ModbusRTUMasterExample.md).

## Hardware Required
- 1 Arduino Board
- Modbus RTU slave/server device(s)
- Possibly an RS-232 or RS-485 transceiver
